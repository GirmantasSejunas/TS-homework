...
const numArr: number[] = [1, 2, -3, 5, 8, 22, 65, -98, 854, 24, -666, 845, ];
const wordArr: string[] = ['namas', 'masina', 'saule', 'debesys'];
console.log(numArr);
console.log(wordArr);

console.groupCollapsed('1. Parašykite funkciją, kuri atspausdina paskutinio masyvo elemento indeksą');
{
  const lastIndexOfArr = (arr: any[]) => (arr.length - 1);
...
{
  const allArrIndex = (arr: any[]): void => {
...
// [2] => reikšmė
{
    const arrIndexAndValues = (arr: any[]) => {
...
{
  const reversedArrValues = (arr: any[]): void => {
    for (let i = 0; i < arr.length; i++) console.log(`${arr[i]}`)
  }
  console.log(numArr);
...
{
  const arrIndexInaRow = (arr: any[]): void => {
...
//  [0]=>17, [1]=>8, [2]=>88 ..
{
    const arrIndexAndValuesInRow = (arr: any[]) => {
...
{
  const doubleArr = (arr: number[]): number[] => arr.map(nr => nr * 2);
  console.log(numArr)
...
{
  const squereArrValues = (arr: number[]): number[] => arr.map(x => x * x);
  console.log(numArr);
...
{
  const arrValueAndIndexMultiplication = (arr: number[]): number[] => arr.map((e, i) => e * i);
  console.log(numArr);
...
{
  const onlyPositiveNumbers = (arr: number[]): number[] => arr.filter(n => n > 0);
...
{
    const onlyNegativeNumbers = (arr: number[]): number[] => arr.filter(n => n < 0);
...
{
  const evenNumbersArr = (arr: number[]):number[] => arr.filter((x) => x % 2 === 0);
    console.log(numArr)
...
{
  const oddNumbersArr = (arr: number[]): number[] => arr.filter((e)=> e % 2 != 0);
  console.log(numArr)
...
{
  const onlyPositiveNumbersOfArr = (arr: number[]): number[] => arr.map((x) => x < 0 ? (x * (-1)) : x);
